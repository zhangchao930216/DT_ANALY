...
public class TrainSameU2_2Map extends Mapper<LongWritable, Text, Text, Text> {
...
//		Path[] paths = DistributedCache.getLocalCacheFiles(context.getConfiguration());
		BufferedReader in = new BufferedReader(new FileReader(new Path("/root/output/u1/u1Mapping/u1ImsiMapping-r-00000").toString()));
//		BufferedReader in = new BufferedReader(new FileReader(paths[1].toString()));
	...
	protected void map(LongWritable inKey, Text value,Context context)throws IOException, InterruptedException {
		...
		if(value.getLength() > 0){
			val = value.toString().split(StringUtils.DELIMITER_INNER_ITEM);
