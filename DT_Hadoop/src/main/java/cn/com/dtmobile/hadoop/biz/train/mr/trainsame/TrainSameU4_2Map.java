...
public class TrainSameU4_2Map extends Mapper<LongWritable, Text, Text, Text> {
...
//		Path[] paths = DistributedCache.getLocalCacheFiles(context.getConfiguration());
		BufferedReader in = new BufferedReader(new FileReader(new Path("/root/output/u3/u3Mapping/u3ImsiMapping-r-00000").toString()));
//		BufferedReader in = new BufferedReader(new FileReader(paths[1].toString()));
	...
	if(imsiMapping.containsKey(val[8])){
		...
		context.write(key,new Text(value.toString() + StringUtils.DELIMITER_INNER_ITEM + imsiMapping.get(val[8] + StringUtils.DELIMITER_INNER_ITEM + imsiMapping.)));
	}*/
}
